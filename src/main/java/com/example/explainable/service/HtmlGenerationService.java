...
public class HtmlGenerationService {
    ...
    public GeneratedUi generate(String prompt) {
        ...
        String themeText = dark ? "#f9fafb" : "#0f172a";
        String themeAccent = dark ? "#60a5fa" : "#2563eb";
        ...
        css.append(".wrap{max-width:960px;margin:0 auto;padding:32px;}");
        css.append(".hero{background:").append(themeSurface).append(";border:1px solid ").append(border).append(";border-radius:20px;padding:28px;box-shadow:0 10px 24px rgba(0,0,0,.08);}");
        ...
        css.append(".input{width:100%;padding:12px 14px;border-radius:12px;border:1px solid ").append(border).append(";margin-top:10px;box-sizing:border-box;}");
        css.append(".muted{opacity:.8;}");
        if (card) {
        ...
        if (login) {
            html.append("<form class='card' style='max-width:420px'>")
                .append("<h2>Login</h2>")
                .append("<input class='input' placeholder='Email' />")
                .append("<input class='input' type='password' placeholder='Password' />")
                .append("<div style='margin-top:14px'><button class='btn' type='button'>Sign in</button></div>")
                .append("</form>");
        } else if (todo) {
            html.append("<div class='card card-highlight'>")
                .append("<h2>Tasks</h2>")
                .append("<input class='input' placeholder='Add task' />")
                .append("<div style='margin-top:14px'><button class='btn'>Add task</button></div>")
                .append("</div>");
        } else {
            html.append("<div class='row'>")
                .append("<div class='card'><h2>Main section</h2><p>Describe your idea more precisely to get a better layout.</p><a class='btn' href='#'>Primary action</a></div>")
                .append("<div class='card'><h2>Details</h2><p class='muted'>This area reflects the detected structure of the prompt.</p></div>")
                .append("</div>");
        }

        html.append("</section>");
        html.append("<div class='row'>")
            .append("<div class='card'><h3>Feature A</h3><p>Auto-generated content block.</p></div>")
            .append("<div class='card'><h3>Feature B</h3><p>Auto-generated content block.</p></div>")
            .append("</div>");
        html.append("</div></body></html>");

        return new GeneratedUi(html.toString(), css.toString(), title);
    }
